...
이 프로젝트는 OCR 출력 결과에 포함된 레이아웃 및 배치 특성을 분석하여,
원래는 분리되어 있던 텍스트 행들을 의미적으로 연속된 문단으로 자동 통합함으로써
의료 텍스트의 가독성, 검색성, API 호출 및 후속 자연어 처리(NLP) 환경에서의 활용도를 향상시킵니다.

⚠️ **제한 사항:**

본 프로젝트는 레이아웃 특성에 기반한 방식으로 OCR 출력 결과의 문단을 재구성합니다.
스캔 문서는 문서 배치, 글꼴, 행간 등이 크게 달라질 수 있기 때문에,
**해당 알고리즘은 모든 문서 형식에서 안정적으로 동작한다고 보장할 수는 없습니다.**

현재 방식은 **설명 가능하고, 조정 가능하며, 제어 가능한 문단 재구성의 기준선(baseline)**을 제공하는 것을 목표로 하며,
향후 보다 복잡한 레이아웃 모델링이나 학습 기반 방법을 위한 참고 구현으로 활용될 수 있습니다.

실제 사용 시에는 문서 유형에 따라 일부 파라미터를 미세 조정해야 할 수 있습니다. 예를 들면 다음과 같습니다:

* `indent_threshold`：문단 들여쓰기를 판단하기 위한 임계값
* `line_gap_threshold`：행 내 줄바꿈과 문단 간 간격을 구분하기 위한 임계값

또한 문서 형식에 따라 아래와 같은 방식으로 출력 처리를 교체하여
더 나은 결과를 얻을 수 있습니다:
